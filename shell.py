...
import getopt
import logging
from fastdnspy.common import to_bytes, to_str, IPNetwork, PortRange
from fastdnspy import encrypt
...
def __version():
    ...
    try:
        ...
        version_str = pkg_resources.get_distribution('fastdnspy').version
    except Exception:
        try:
            from fastdnspy import version
...
def print_fastdnspy():
    print('fastdnspyR %s' % __version())

def log_fastdnspy_version():
    logging.info('fastdnspyR %s' % __version())
...
def get_config(is_local):
    ...
    config['pid-file'] = config.get('pid-file', '/var/run/fastdnspyr.pid')
    config['log-file'] = config.get('log-file', '/var/log/fastdnspyr.log')
...
Online help: <https://github.com/fastdnspy/fastdnspy>
''')
